// legend.js - Functions for creating and updating the map legend

/**
 * Creates or updates the legend for the current dimension
 * @param {string} dimension - The current dimension being displayed
 */
export function updateLegend(dimension) {
  ...
  const scheme = getLegendScheme(dimension);
...
/**
 * Gets the legend scheme for the specified dimension
 * @param {string} dimension - The dimension to get legend for
 * @returns {Object} Legend configuration with title and ranges
 */
function getLegendScheme(dimension) {
  const legendSchemes = {
    "System Performance": {
  ...
  return legendSchemes[dimension] || legendSchemes["System Performance"];
}